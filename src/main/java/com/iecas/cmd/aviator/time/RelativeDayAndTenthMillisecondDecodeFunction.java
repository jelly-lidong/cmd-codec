...
/**
 * 反向计算：根据基准时间、积日和积0.1毫秒计算目标时间
 * <p>编码格式：前2字节存储积日，后4字节存储积0.1毫秒（相对当天零点）</p>
 *
 * @author CMD Team
 * @version 1.0
 * @since 2024-01-01
 */
public class RelativeDayAndTenthMillisecondDecodeFunction extends AbstractFunction {
     ...
     * 构造函数
     */
    public RelativeDayAndTenthMillisecondDecodeFunction() {
        super();
    }
    ...
    public String getName() {
        return "relativeDayAndTenthMillisecondDecode";
    }
    ...
    public AviatorObject call(Map<String, Object> env, AviatorObject arg1, AviatorObject encodedValue) {
        try {
            ...
            // 单位:0.1毫秒
            long tenthMilliseconds = dayAndMillisecond[1];

            // 计算目标时间
            String targetTime = calculateTargetTime(baseDateTime, days, tenthMilliseconds);
     ...
     * @param days         积日
     * @param tenthMilliseconds 积毫秒
     * @return 目标时间字符串
     */
    private String calculateTargetTime(LocalDateTime baseDateTime, long days, long tenthMilliseconds) {
        // 验证参数范围
        if (days < -MAX_DAYS || days > MAX_DAYS) {
            throw new IllegalArgumentException("积日超出范围: " + days + "，允许范围: [" + (-MAX_DAYS) + ", " + MAX_DAYS + "]");
        }
        if (tenthMilliseconds < 0 || tenthMilliseconds > MAX_MILLISECONDS) {
            throw new IllegalArgumentException("积0.1毫秒超出范围: " + tenthMilliseconds + "，允许范围: [0, " + MAX_MILLISECONDS + "]");
        }
        ...
        // 计算目标时间（当天零点 + 积0.1毫秒）
        LocalDateTime targetDateTime = targetDate.toLocalDate().atStartOfDay().plusNanos(tenthMilliseconds * 1_000_00);
